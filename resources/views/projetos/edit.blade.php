        ...
        <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden sm:rounded-lg">
            ...
            <form method="POST" action="{{ route('projetos.update', $projeto) }}" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <select name="client_id" id="client_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        @foreach ($clientes as $cliente)
                            <option value="{{ $cliente->id }}" {{ old('client_id', $projeto->client_id) == $cliente->id ? 'selected' : '' }}>{{ $cliente->id }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    ...
                    <textarea id="description" name="description" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $projeto->description) }}</textarea>
                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                </div>

                <div class="mt-4">
                    <x-input-label for="icone" :value="__('Ícone (opcional)')" />
                    @if ($projeto->icone)
                        <img src="{{ Storage::url($projeto->icone) }}" alt="{{ $projeto->name }} Ícone" width="100" class="mb-2">
                    @endif
                    <input type="file" id="icone" name="icone" class="block mt-1 w-full">
